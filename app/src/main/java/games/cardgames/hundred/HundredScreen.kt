...
fun HundredScreen(
    ...
    LaunchedEffect(Unit) {
        when {
            ...
            // Кон сыгран и ждёт ответа — а игрок тем временем уходил в меню и
            // вернулся. lastPhrase хранит объявление конца кона целиком, и на
            // возврате оно звучало бы как только что случившееся: последний ход
            // и подсчёт очков заново (жалоба Катерины, 20.09). За столом в этот
            // миг решается не ход, а играть ли дальше, поэтому говорим ровно то
            // же, что и при подъёме стола с диска.
            session.match.awaitingDeal() && !session.overForPlayer ->
                voice.say(resumePhrase(session.match, names), whenReady = true)
