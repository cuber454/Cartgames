...
/**
 * Голос соперника за столом.
 *
 * [apart] — слышно ли его отдельно от приложения: выбран свой синтезатор или
 * свой голос (см. `botSpeaksAlone`). Такому сопернику реплика идёт его
 * синтезатором и при работающем скринридере; остальным — как решит [Speech].
 */
class BotVoice(val speaker: Speaker, val apart: Boolean)
...
class TableVoice(
     ...
     */
    private val botSpeakers: Map<Int, BotVoice> = emptyMap(),
     ...
     *
     * Бот со своим голосом говорит им и при работающем скринридере: тот
     * озвучивает приложение, а соперник — своим синтезатором. Иначе выбранный
     * ему голос за столом не звучит вовсе: скринридер читает его фразу своим,
     * единственным на всё (Катерина, 19.09: «давай доделаем так, чтобы боты
     * говорили отдельно и когда включён Screen Reader, а самоозвучка не
     * включалась»). Перед своей фразой синтезатор приложения велит скринридеру
     * умолкнуть — иначе две речи налезут друг на друга (см. [Speaker.say]).
     *
     * Бот без своего голоса остаётся за скринридером: говорит он тем же
     * голосом, что и приложение, и своя озвучка здесь означала бы ровно то,
     * чего быть не должно, — включившуюся самоозвучку.
     *
    ...
    fun sayBot(text: String, seat: Int = FIRST_BOT_SEAT, whenReady: Boolean = false, afterMs: Long = 0L) {
        val bot = botSpeakers[seat]
        val voice = bot?.speaker ?: speaker
        note(text, afterMs, voice = voice)
        // «Никто» — молчат и боты: это игра для зрячих, и читать им нечего.
        val now = speech()
        if (now == Speech.NONE) return
        val who = if (bot?.apart == true) Speech.APP else now
        if (afterMs <= 0) {
            sayEvent(view, voice, who, text, whenReady)
            return
        }
        scope.launch {
            delay(afterMs)
     ...
     * синтезатором.
     *
     * Спрашивают, чтобы решить, называть ли бота по имени. Своим голосом бот
     * говорит о себе «я»: имя в его собственной речи звучало бы справкой о
     * нём, а не речью (Катерина, 19.09: «получается, как будто бы говорит в
     * третьем лице»). Когда его фразу читает скринридер, голос у него общий со
     * всем остальным, и реплика без имени не говорит, чья она, — там имя и
     * остаётся.
     */
    fun speaksAlone(seat: Int = FIRST_BOT_SEAT): Boolean =
        botSpeakers[seat]?.apart == true || speech().speaks
